import sys
from scrambler import encode, generate_key
...
try: 
    ...
    print('Nie udało otworzyć się pliku')
    sys.exit()

encryption_key = generate_key(int(key_length))
...
for i, letter in enumerate(symbols):
    compression_keys.append({'letter' : letter, 'bin_rep': format(i, f'0{bit_amount}b')})

for letter in text_data[0]:
    compressed_message += get_bin_rep(letter, compression_keys)
...
with open('compressed.txt', 'wb') as f:
    ...
    f.write(encode(encryption_key, int_list))

print(f'Komprimowanie zakończone sukcesem! Zapisano do pliku compressed.txt')
print(f'Klucz do dekrypcji: {encryption_key.hex()}')